...
- **通知钩子改动**：改 `hooks/scripts/notify.js` 后结束一次会话，观察系统通知是否弹出；设 `TUANZII_NOTIFY_ENABLED=false` 可验证关闭路径
