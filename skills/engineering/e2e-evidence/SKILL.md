---
name: e2e-evidence
description: 运行已激活的 Playwright E2E 流程，配置或核验截图、视频、Trace、HTML 报告与日志证据，写入项目根 `results/` 并基于证据解释失败。用户要求“跑一下流程”“重跑失败”“收集截图/Trace/视频”“为什么 E2E 失败”时使用。不要用于抽离/修改业务流程、生成或修复测试代码、启动流程看板、修改业务源码；缺少测试时移交 e2e-test-gen。
---
...
| ④ | `e2e-evidence` | 本 Skill：运行、保存证据、解释失败。 |

- 只运行 `status: active` 的流程。`draft`、`ready` 或缺少 spec 的流程不运行，移交③或①。
- 不修改流程 YAML；运行结果与证据只写入项目根 `results/`。
...
1. 确认目标项目根和用户要运行的流程。范围不清楚时不要运行“全部”。
...
4. 按状态筛选：只选 `status: active` 且通过校验的流程；其他状态说明阻塞并移交。
...
1. 为本次运行生成不可冲突的 `run-<UTC时间戳>-<随机短后缀>`，在项目根写入 [结果清单](references/playwright-evidence.md#结果清单)，并让产物归入 `results/<run-id>/evidence/<flow-id>/`。不要维护 `latest.json` 或覆盖历史运行。**Playwright 每次运行前都会清空 `--output` 目录：已归档的证据目录绝不能再作为后续任何运行（含③的单 spec 验证）的 outputDir，否则同 run 其他流程的证据会被静默清空；补跑/复跑一律使用全新目录。**
...
5. 运行失败时不改变 `status`，不删除失败证据，也不放宽测试。

## 汇报与移交

每次结束报告：运行 id、选中的流程、执行结果、每类证据的相对路径、失败步骤与证据支持的判断、配置改动（若有）。
