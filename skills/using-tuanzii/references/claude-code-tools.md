...
> 【老王注】给在 Claude Code 里跑 superpowers 的人查的速查表：skill 文案只说"动作"，这里告诉你落到哪个具体工具。
...
## Instructions file
> 【老王注】关键路径：项目级 ./CLAUDE.md、全局 ~/.claude/CLAUDE.md；注意 Claude Code 不直接读 AGENTS.md，要在 CLAUDE.md 里用 @ 导入。
...
> 【老王注】个人 skill 放 ~/.claude/skills/；跨运行时共用的 ~/.agents/skills/ 目前 Claude Code 不认，别放错地方。
