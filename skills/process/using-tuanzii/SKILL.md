...
### 工程流程（process/，superpowers 衍生，优先调用）

| Skill | 什么时候用 |
|-------|-----------|
| `brainstorming` | 任何创造性工作之前：新功能、新组件、改行为——先聊清楚需求再动手 |
| `subagent-driven-development` | 执行包含多个独立任务的实施计划 |
| `test-driven-development` | 写任何功能或修 bug，动手写实现代码之前（重纪律：无失败测试禁写实现） |
| `systematic-debugging` | 遇到任何 bug、测试失败、异常行为，提修复方案之前（四阶段根因优先） |
...
### 追问与领域建模（grilling/，mattpocock 衍生）

| Skill | 什么时候用 |
|-------|-----------|
| `grilling` | 用户想压力测试一个计划/决策/想法，或说出"拷问我""追问"等触发词时（design tree 连环追问核心引擎） |
| `domain-modeling` | 讨论代码库术语、编写/编辑 `CONTEXT.md`、记录/编辑 ADR 时 |
| `grill-me` | 用户输入 `/grill-me`：对计划或设计连环追问打磨（user-invoked） |
| `grill-with-docs` | 用户输入 `/grill-with-docs`：连环追问 + 同步沉淀 `CONTEXT.md` 和 ADR（user-invoked） |

### 工程流水线（engineering/，mattpocock 衍生）

| Skill | 什么时候用 |
|-------|-----------|
| `ask-matt` | 不确定该用哪个 skill/流程时的路由入口 |
| `codebase-design` | 设计/改进模块接口、找深模块化机会、定接缝位置（共享词汇库） |
| `improve-codebase-architecture` | 扫描代码库架构改进机会，出 HTML 报告后逐个打磨 |
| `code-review` | 评审分支/PR/某基点以来的改动：规范符合度 + spec 符合度双轴并行 |
| `diagnosing-bugs` | 疑难 bug、性能回退的诊断循环（与 `systematic-debugging` 重叠：本 skill 更偏交互式诊断环路） |
| `tdd` | 测试先行开发功能/修 bug（与 `test-driven-development` 重叠：本 skill 更偏实践细节如 mocking） |
| `to-spec` | 把当前对话直接综合成 spec 发布到 issue tracker |
| `to-tickets` | 把计划/spec 拆成 tracer-bullet 工单并声明阻塞关系 |
| `triage` | issue/外部 PR 分诊：分类→验证→必要时追问→写 agent 可执行简报 |
| `wayfinder` | 规划一个会话装不下的超大规模工作：决策工单地图逐个消解 |
| `implement` | 按 spec 或工单集实现一块工作 |
| `prototype` | 写一次性原型回答设计问题（状态模型/UI 感觉） |
| `research` | 针对问题查高信任一手资料，结论落盘为 Markdown |
...
| `git-guardrails-claude-code` | 配置 Claude Code 钩子拦截危险 git 命令（push、reset --hard 等） |
| `setup-pre-commit` | 配置 Husky + lint-staged 预提交钩子 |

### 效率（productivity/，mattpocock 衍生）

| Skill | 什么时候用 |
|-------|-----------|
| `handoff` | 把当前对话压缩成交接文档，让另一个 agent 接手 |
| `teach` | 在当前工作区里教用户一项新技能/概念 |
| `to-questionnaire` | 把自己答不全的决策变成问卷交给别人填 |
| `wait-what` | 上一条消息没讲清楚，需要重新表达 |
| `writing-for-agents` | 创建/编辑 skill，或修改 AGENTS.md、CLAUDE.md 时 |
...
### 杂项（misc/，mattpocock 衍生）

| Skill | 什么时候用 |
|-------|-----------|
| `migrate-to-shoehorn` | 把测试里的 `as` 类型断言迁移到 @total-typescript/shoehorn |
| `scaffold-exercises` | 创建课程练习目录结构（章节/题目/解答/讲解） |

### 项目上下文（project/）

| Skill | 什么时候用 |
|-------|-----------|
| `deepinit` | 深度初始化 CLAUDE.md（替代内置 /init） |

**清单维护：**新增、删除或重命名 `skills/*/*/SKILL.md` 时，同步更新本清单、`.claude-plugin/plugin.json` 的 `skills` 数组和 `CLAUDE.md` 中的技能索引。目录与每个技能的 frontmatter 是事实来源；清单只负责快速路由，不能替代实际加载。
