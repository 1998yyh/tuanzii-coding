...
> 【老王注】九步按顺序走：摸底 → 按需提可视化 → 一次一问澄清 → 给 2-3 个方案 → 分段过设计 → 落盘 → 自审 → 用户审 → 产出实施计划。
> 【老王注】writing-plans 已移除（模型原生能力）：第 9 步不再调用专门 skill，直接产出细粒度实施计划（拆任务、每步写清代码/命令/验证方式）并按需执行。
...
> 【老王注】writing-plans 已移除：设计获批后的“下一步”改为直接产出细粒度实施计划并进入实现，不再有专门 skill 承接（上图 dot 图中 “Invoke writing-plans skill” 同理）。
> 【老王注】终点：设计获批即进入实施计划环节，脑子里冒出"反正设计清楚了，直接开写"的念头时，回来读这句。
...
> 【老王注】writing-plans 已移除：此步改为直接制定细粒度实施计划（拆任务、每步写清代码/命令/验证方式）并进入实现。
> 【老王注】别自作主张直接开写或改调别的 skill——计划环节是设计到实现的必经一步。
