...
// 【老王注】brainstorm 伴生服务器：零依赖 Node HTTP + 手写 WebSocket 服务。agent 往 content 目录丢 HTML，浏览器实时刷新展示。
// 【老王注】由 start-server.sh 拉起，端口/目录/token 全部经 BRAINSTORM_* 环境变量传入；启动成功后 stdout 打一行 server-started JSON。
// 【老王注】输入：content 目录的 .html 文件 + 浏览器 WebSocket 回传事件；输出：JSON 状态行（stdout）+ state 目录下的事件文件。
// 【老王注】token 鉴权、Origin 校验、路径校验是安全底线，改动前先把相关英文原注释读明白。
...
// 【老王注】WebSocket 握手核心：sha1(客户端 key + 固定魔法串) 得 accept key，RFC 6455 的规定动作。
function computeAcceptKey(clientKey) {
  return crypto.createHash('sha1').update(clientKey + WS_MAGIC).digest('base64');
}

// 【老王注】按 payload 长度选 2/4/10 字节帧头；只发完整帧不分片（FIN 恒为 1）。
...
// 【老王注】解析客户端帧：数据不够返回 null 等下一段；客户端帧必须带 mask，超长直接抛错由上层断连。
...
// 【老王注】端口优先级：环境变量 > 上次记录的端口 > 随机高端口；重启复用是为了已打开的标签页还能连上。
...
// 【老王注】token 来源：环境变量 > token 文件 > 现场生成；从文件读到时顺手把权限收紧成 600。
...
// 【老王注】403 页面：提示用户必须拿带 ?key= 的完整 URL 来访问。
...
// 【老王注】带 key 首访时先回这个引导页：key 存进 sessionStorage 再跳回裸 /，地址栏不留 token。
...
// 【老王注】品牌栏：遥测被禁用时换文案且不挂 logo 图，避免多余的外联请求。
...
// 【老王注】取 content 目录里 mtime 最新的 .html 作为当前屏幕，异常文件（软链等）直接过滤。
...
// 【老王注】按平台选开浏览器的命令；无图形环境返回 null，headless 就不开了。
...
// 【老王注】防穿越/防软链：必须是 content 目录内的普通文件、非软链、硬链接数为 1，缺一不收。
...
// 【老王注】常量时间比较，防时序侧信道猜 token；长度不同直接 false（timingSafeEqual 要求等长）。
...
// 【老王注】鉴权总闸：?key= 或会话 cookie 二选一，比对走常量时间；不过就是 403，别心软。
...
// 【老王注】统一安全响应头：禁 referer 泄漏、禁缓存、禁被 iframe 嵌。
...
// 【老王注】WS 的 Origin 必须恰好等于 http://host，挡跨站 localhost 注入；无 Origin（非浏览器）放行。
...
// 【老王注】HTTP 总入口：先鉴权（失败 403），再按路径分发 /、/files/*，其余一律 404。
...
// 【老王注】带 key 首访走引导页，把 token 从地址栏挪进 sessionStorage。
  if (req.method === 'GET' && pathname === '/' && keyFromQuery && timingSafeEqualStr(keyFromQuery, TOKEN)) {
    res.writeHead(200, securityHeaders({ 'Content-Type': 'text/html; charset=utf-8' }));
    res.end(bootstrapPage(keyFromQuery));
  } else if (req.method === 'GET' && pathname === '/') {
// 【老王注】主页：有最新屏幕就推（整页直回、片段套模板），并在 body 收尾处注入 helper.js。
...
// 【老王注】basename + 这一串检查专门堵路径穿越和 EISDIR 崩溃，别嫌啰嗦。
...
// 【老王注】WS 升级入口：鉴权 + Origin 双检查，不过直接 destroy，连 403 都懒得回。
...
// 【老王注】浏览器事件入口：打 JSON 日志；带 choice 的追加写进 state/events，agent 靠它收用户选择。
...
// 【老王注】向所有已连客户端广播（主要是 reload）；写失败的连接顺手从集合里清掉。
...
// 【老王注】只在首个屏幕就绪时开一次浏览器；BRAINSTORM_OPEN 是 opt-in，默认不开。
...
// 【老王注】启动主流程：建目录 → 起 HTTP/WS 服务 → 盯 content 目录变化 → 装生命周期看门狗。
...
// 【老王注】盯 content 目录：新 .html 触发开浏览器 + 广播 reload，100ms 去抖防重复触发。
...
// 【老王注】优雅退出：写 stopped 标记、删 server-info、毁掉所有 WS 连接，否则 server.close() 永远等不完。
...
// 【老王注】kill(pid, 0) 探测宿主活没活；EPERM 说明活着但无权发信号，也算活。
  function ownerAlive() {
...
// 【老王注】绑定成功：cookie 名按实际端口定；只在拿到首选端口时回写 port/token 文件，回退端口时别覆盖别人的会话。
  function onListen() {
...
// 【老王注】端口被占回退一次随机端口；token 来自环境变量时拒绝回退（会跟别人串号），直接退出。
  server.on('error', (err) => {
...
// 【老王注】导出仅供单元测试；正常运行走上面的 require.main 分支。
module.exports = {
  computeAcceptKey,
  encodeFrame,
  decodeFrame,
