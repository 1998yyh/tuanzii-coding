...
description: "为文章、演示或技术文档生成水彩墨线风格的技术插图与信息图。用户要求技术配图、流程图、架构图、方案对比、机制或概念解释图、数据故事配图时使用；需要可编辑 Mermaid/SVG 或精确数据绘图时使用对应工具。"
---
...
可通过 `/tuanzii:technical-image-generation` 调用，随后描述主题、读者和必须保留的文字。模板和示例路径均相对于本 skill 目录，按需读取选中的模板；需要参考风格时查看 `assets/` 中对应示例。

本 skill 提供构图、提示词与验收规则，实际出图使用当前环境可用的图像生成工具，并遵循该工具的调用要求。若环境没有生图能力，交付完整提示词和建议文件名，明确尚未生成图片。生成产物写入用户指定目录；未指定时使用当前工作项目的 `results/technical-images/`，保留既有版本。
...
3. 用下方 Prompt 框架生成图片。
4. 读取 [质量检查表](references/quality-checklist.md) 并查看成图，检查文字、关系和版面；一次只修改一个问题。连续三轮无改善时缩小范围或拆图。
