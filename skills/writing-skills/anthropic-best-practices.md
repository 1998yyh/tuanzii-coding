...
## Core principles
> 【老王注】全文的纲：三条核心原则——简洁、自由度匹配任务、在所有目标模型上实测。
> 【老王注】核心一句话：上下文窗口是公共资源，skill 的每个 token 都在跟对话历史和其他 skill 抢地方。

### Concise is key
> 【老王注】简洁是第一位：启动只预载所有 skill 的 name+description，SKILL.md 被触发才读，附属文件按需读。
> 【老王注】默认 agent 已经很聪明，只写它不知道的；每段话都自问值不值这个 token 成本。
...
> 【老王注】自由度要匹配任务脆弱度：越怕出错的操作指令越写死（低自由度），越开放的任务越只给方向（高自由度）。
> 【老王注】原文的比喻：窄桥两边是悬崖就装护栏给死命令，空旷原野就给指南针让 agent 自己走。
...
> 【老王注】skill 效果依赖底层模型：打算用哪个模型就在哪个上测；Opus 嫌啰嗦的详细度，Haiku 可能正需要。
...
## Skill structure
> 【老王注】这节讲 SKILL.md 的骨架：frontmatter 必填 name（≤64 字符）和 description（≤1024 字符）。
> 【老王注】重点在渐进式披露——SKILL.md 当目录用，细节拆文件按需加载。
...
### Naming conventions
> 【老王注】命名推荐动名词（verb+ing，如 Processing PDFs）；避开 Helper/Utils/Tools 这类废名，同一个 skill 库里风格要统一。
...
> 【老王注】description 是 agent 从上百个 skill 里挑中你的唯一依据：写清“做什么+什么时候用”，带上触发关键词。
> 【老王注】必须第三人称（“Processes Excel files”），写“我能帮你…”会干扰 skill 发现。
...
> 【老王注】渐进式披露：SKILL.md 只当目录和导航，正文压到 500 行内，细节拆进独立文件按需加载。
> 【老王注】下面给了三种组织模式——指南+引用、按领域拆目录、基础内容+进阶链接，按 skill 复杂度选。
...
> 【老王注】引用只许一层深：所有参考文件直接从 SKILL.md 链出；套娃式链接会让 agent 只预览个开头，信息就丢了。
...
> 【老王注】超过 100 行的参考文件开头放目录，agent 即使只 head 预览也能看到全貌、按节跳读。
...
> 【老王注】复杂任务两大件：编号步骤的工作流（附可勾选 checklist），加“验证→修错→重跑”的反馈环。
> 【老王注】目的只有一个：防 agent 跳步、让错误尽早暴露。

### Use workflows for complex tasks
> 【老王注】工作流写法：步骤编号、顺序明确；复杂流程给可勾选清单让 agent 边做边打勾，防跳步（尤其验证步）。
...
> 【老王注】反馈环是提质量的关键套路：跑校验→修错→再跑，全过才往下走；校验器可以是脚本，也可以是供对照的风格指南。
...
## Content guidelines
> 【老王注】内容层面两条规矩：别写会过期的时效信息，术语全文统一。
> 【老王注】都是防“skill 用着用着就误导 agent”的低级坑。

### Avoid time-sensitive information
> 【老王注】别写“2025 年 8 月前用旧 API”这种会过期的句子；旧做法挪进 Old patterns 折叠区，正文只讲当前做法。
...
> 【老王注】一个概念只用一个词：全文统一叫 API endpoint，别 endpoint/URL/route 混着来，混用会让 agent 理解跑偏。
...
## Common patterns
> 【老王注】三个即插即用的套路：模板（定输出格式）、示例（给输入输出对）、条件工作流（按场景分流）。
> 【老王注】写新 skill 时先看这三个模式够不够套，够就不用自己造结构。

### Template pattern
> 【老王注】模板模式：输出格式有硬要求就给死模板，允许发挥就给“建议默认格式”，严格程度按需求定。
...
### Examples pattern
> 【老王注】示例模式：输出吃风格的 skill 直接给几对输入/输出样例，比用文字描述风格管用得多。
...
> 【老王注】条件工作流：先让 agent 判断场景（新建还是编辑？），再分流到对应子流程；流程太大就拆文件按需读。
...
> 【老王注】怎么证明 skill 真有用：评测先行，再用 A/B 双 agent 循环迭代——A 负责写，B 负责跑真实任务。
> 【老王注】改进依据是真实使用观察，不是拍脑袋假设。

### Build evaluations first
> 【老王注】评测先行：先不装 skill 跑任务、记录具体失败点，据此造至少 3 个评测场景，再写最小内容把评测跑过。
> 【老王注】这样保证解决的是真实问题，而不是为想象中的需求写文档。
...
> 【老王注】双 agent 迭代法：Agent A 帮你把刚做完的任务沉淀成 skill，Agent B（全新实例）拿去跑真实任务。
> 【老王注】观察 B 卡在哪，带着具体现象回来让 A 改，循环往复；团队同事的使用反馈也一样收。
...
> 【老王注】盯 agent 实际怎么用 skill：读文件顺序意外、漏跟引用、反复读同一文件、从不碰某文件——都是结构问题的信号，照着改。
...
> 【老王注】反面清单，两条：别用 Windows 反斜杠路径，别一次甩一堆并列选项。
> 【老王注】后面进阶篇还有“别假设依赖已安装”等同款坑。

### Avoid Windows-style paths
> 【老王注】路径一律正斜杠 scripts/helper.py；反斜杠在 Unix 上直接报错。
...
> 【老王注】别罗列四五个并列库让 agent 挑；给一个默认推荐，再留一个例外出口（如“扫描件 OCR 就用 pdf2image”）。
...
> 【老王注】进阶篇：skill 里带可执行脚本时的写法——错误处理、内置工具脚本、视觉分析、计划校验、依赖与运行时。
> 【老王注】纯 markdown 的 skill 可以跳过这节，直接去看文末清单。

The sections below focus on Skills that include executable scripts. If your Skill uses only markdown instructions, skip to [Checklist for effective Skills](#checklist-for-effective-skills).

### Solve, don't punt
> 【老王注】脚本要自己兜住错误（文件不存在就建默认、没权限就给替代），别把异常甩给 agent 收拾。
> 【老王注】常量也要注释说明取值理由，拒绝 TIMEOUT=47 这种魔法数字。
...
> 【老王注】即使 agent 自己能写脚本，也优先内置成品：更可靠、省 token、结果一致；指令里写清脚本是“直接跑”还是“当参考读”。
...
### Use visual analysis
> 【老王注】输入能渲染成图（如 PDF 转图片）时，让 agent 用视觉能力直接看版面，比纯文本解析直观。
...
> 【老王注】高危/批量操作用“计划→校验→执行”模式：先生成结构化计划文件（如 changes.json），脚本校验通过再真执行。
> 【老王注】错误在落地前被拦住；校验脚本的报错要具体到字段名，方便 agent 自己修。
...
> 【老王注】依赖提醒：claude.ai 能装 npm/PyPI 包，API 环境没网装不了；所需包要在 SKILL.md 列明并核对目标环境支持。
...
### Runtime environment
> 【老王注】运行时机制：元数据预载、文件按需读、脚本只耗输出的 token、大文件不读不占上下文。
> 【老王注】这套文件系统式架构就是渐进式披露能成立的地基，路径、命名、目录组织都要按“给 agent 导航”来设计。
...
### MCP tool references
> 【老王注】引用 MCP 工具必须写全名 ServerName:tool_name（如 BigQuery:bigquery_schema），缺服务器前缀容易 tool not found。
...
> 【老王注】别假设环境里装了某个库：先写安装命令（pip install pypdf），再给用法。
...
## Technical notes
> 【老王注】两条硬指标速查：frontmatter 字段长度上限、正文 500 行预算。
> 【老王注】和前文呼应，放这里方便临走前再核对一遍。

### YAML frontmatter requirements
> 【老王注】硬性要求：name ≤64 字符、description ≤1024 字符，两个都必填。
...
### Token budgets
> 【老王注】SKILL.md 正文压在 500 行内，超了就按渐进式披露拆文件。
...
> 【老王注】发布前自检清单，分核心质量、代码脚本、测试三块。
> 【老王注】分享 skill 前逐条过一遍，比凭记忆靠谱。

Before sharing a Skill, verify:

### Core quality
> 【老王注】质量项：描述具体带关键词、正文 500 行内、无时效信息、术语一致、引用一层深、工作流步骤清晰。
...
### Code and scripts
> 【老王注】脚本项：自己兜错、无魔法数字、依赖列明、全正斜杠路径、关键操作有验证和反馈环。
...
### Testing
> 【老王注】测试项：至少 3 个评测、Haiku/Sonnet/Opus 全测过、真实场景跑过、吸收过团队反馈。
...
## Next steps
> 【老王注】收尾三张链接卡：快速上手、Claude Code 里用、API 里用。
> 【老王注】真动手就从第一张卡的 quickstart 开始。
